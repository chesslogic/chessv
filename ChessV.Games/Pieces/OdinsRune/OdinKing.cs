...
namespace ChessV.Games.Pieces.OdinsRune
{
  public class OdinKing : PieceType
  {
    ...
    public static bool CustomMoveGenerationHandler(PieceType pieceType, Piece piece, MoveList moveList, bool capturesOnly)
    {
      ...
      for (int x = 0; x < 8; x++)
      {
        int square = SafeNextSquare(piece.Board, piece.Player, x, piece.Square);
      ...
      if (foundPawn)
      {
        ...
        else
        {
          ...
          int forwardSquare = SafeNextSquare(game.Board, piece.Player, PredefinedDirections.N + piece.Player, piece.Square);
          if (!foundValkyrie ||
            (forwardSquare >= 0 && game.Board[forwardSquare] != null))
          {
            //	Is the square two ahead both on the board and a square 
            //	we can move to?
            int targetSquare = forwardSquare;
            if (targetSquare >= 0)
              targetSquare = SafeNextSquare(game.Board, piece.Player, PredefinedDirections.N + piece.Player, targetSquare);
            if (targetSquare >= 0 &&
               ...
               (game.Board[targetSquare] != null && game.Board[targetSquare].Player != piece.Player)))
            {
              ...
              int neSquare = SafeNextSquare(game.Board, piece.Player, PredefinedDirections.NE + piece.Player, piece.Square);
              int nwSquare = SafeNextSquare(game.Board, piece.Player, PredefinedDirections.NW + piece.Player, piece.Square);
              if ((neSquare >= 0 && game.Board[neSquare] == null) ||
                 (nwSquare >= 0 && game.Board[nwSquare] == null))
              {
