...
Current-contract ChecksMate worlds can unlock five board profiles: 8x8, 10x8, 10x10, 12x10, and 12x12. `Board Files` and `Board Ranks` advance independently in two-square steps, while the client only offers the valid profile pairs published by the world. The board selector defaults to the largest unlocked profile on connection and advances when a new profile is unlocked; a manually selected smaller board remains available as a handicap and survives unrelated item updates. The selector is locked while a match is active. Legacy worlds retain their original 8x8 / Super-Sized 10x8 behavior.
